...
export const TodoItem: React.FC<TodoItemProps> = ({ todo, onUpdate, onDelete }) => {
  ...
  return (
    <div
      className={`p-5 rounded-2xl shadow-lg mb-4 ${todo.done ? 'bg-gray-700' : ''} bg-gradient-to-r from-purple-800 to-purple-500 text-white`}
    >
      {/* Task Header */}
      <div className="flex justify-between items-center mb-3">
        ...
        <div className="flex items-center space-x-2">
          <button
            onClick={toggleDone}
            className={`px-2 py-1 text-sm rounded ${todo.done ? 'bg-gray-600' : 'bg-purple-600 hover:bg-purple-500'}`}
          >
            {todo.done ? 'Undo' : 'Done'}
          </button>
          <button
      ...
      <ul className="list-disc pl-5 space-y-1 text-sm opacity-90">
        {todo.subTasks.map((sub, index) => (
          <li key={index} className={`flex items-center justify-between ${sub.done ? 'line-through opacity-60' : ''}`}>
            <span>{sub.text}</span>
            <button
              className={`px-2 py-0.5 text-xs rounded ${sub.done ? 'bg-gray-600' : 'bg-purple-600 hover:bg-purple-500'}`}
